# HESS Plastics — Print Server (Windows 11)

Tiskový server pro dvě USB etiketové tiskárny. Webové rozhraní přístupné z libovolného zařízení v síti.

---

## Rychlý start

```
1. Rozbalit ZIP do C:\printserver_win\
2. Spustit install.bat (jako správce)
3. Spustit start.bat
4. Otevřít http://localhost:5000
5. Přihlásit se: admin / admin123
6. Admin → Nastavení → vybrat tiskárny, protokol, DPI
7. Admin → Šablony → vytvořit šablonu štítku
8. Admin → Produkty → přidat produkty a přiřadit šablonu
```

---

## Changelog

### Aktuální změny
- Číslo výrobního příkazu je volitelné. Pole v modalu zůstává, ale prázdná hodnota už neblokuje zahájení výroby.
- Admin → Produkty má tlačítko **DUPLIKOVAT**. Vytvoří kopii produktu se stejnými parametry a novým kódem typu `KOD COPY`, případně `KOD COPY 2`.
- Produktový/admin modal má vnitřní scroll, takže se vejde i na menší obrazovky.
- **UKONČIT VÝROBU** už nezobrazuje potvrzovací okno. Potvrzení zůstává jen u **ZRUŠIT VP**.
- Beze změny databázového schématu — původní `data\printserver.db` lze použít s touto verzí.

---

## Struktura projektu

```
printserver_win/
├── app.py                  # Flask server (Waitress)
├── config.py               # Výchozí konfigurace
├── models.py               # SQLite schéma + helpers
├── printer.py              # TSPL/ZPL generátor, win32print
├── gpio_listener.py        # HTTP / COM port trigger
├── logger.py               # Rotující file logger
├── requirements.txt        # Python závislosti
├── install.bat             # Instalace (spustit jednou)
├── start.bat               # Spuštění serveru
├── reset_db.bat            # Reset databáze na výchozí stav
├── templates/              # Jinja2 HTML šablony
├── static/css/main.css     # Styly (Hess Plastics branding)
├── static/js/
│   ├── main.js             # Sdílené utility
│   ├── index.js            # Logika tisku + scan flow
│   ├── admin.js            # Admin panel logika
│   └── debug.js            # Debug panel logika
├── data/
│   ├── printserver.db      # SQLite databáze (vše)
│   ├── last_print_L.prn    # Poslední PRN levá (debug)
│   ├── last_print_R.prn    # Poslední PRN pravá (debug)
│   └── logs/
│       └── printserver.log # Rotující log soubor
└── tools/
    └── detect_printer.py   # Detekce a test tiskárny
```

---

## Databáze (SQLite)

Vše v jednom souboru: `data\printserver.db`

| Tabulka | Obsah |
|---|---|
| `users` | Uživatelé — username, password_hash, pin_hash, role |
| `products` | Produkty — kód, QR obsah, text1–4, strana, šablona |
| `templates` | Šablony štítků — rozměry, prvky (JSON) |
| `settings` | Nastavení tiskáren (uložené v DB) |
| `production_orders` | Výrobní příkazy — start/stop, počítadla kusů |
| `print_log` | Log každého tisku |

**Seed data** — vkládají se pouze při prázdné DB (první spuštění nebo po reset_db.bat). Restart serveru s existující DB nic nepřidá.

**Zálohování / přenos na jiný počítač** — stačí zkopírovat `data\printserver.db`. Server při kopírování ideálně vypni. Pokud vedle DB existují soubory `printserver.db-wal` nebo `printserver.db-shm`, zkopíruj je také, případně nejdřív server korektně ukonči.

**Reset** — spustit `reset_db.bat`.

**Migrace** — nové sloupce se přidají automaticky při prvním spuštění nové verze. Existující data zůstanou beze změny.

**Aktuální změny bez migrace** — volitelné číslo výrobního příkazu, duplikace produktů, scroll modalů a odstranění potvrzení při ukončení výroby nemění strukturu DB.

---

## Výchozí přístupy

| Uživatel | Heslo | Role |
|---|---|---|
| admin | admin123 | Admin |
| operator1 | hess2025 | Uživatel |

Změňte hesla po prvním přihlášení v Admin → Uživatelé.

---

## Přihlášení

### Operátoři — PIN
Výchozí obrazovka přihlášení zobrazí pole pro PIN. Operátor naskenuje kartu nebo zadá PIN ručně. Po 4+ znacích se automaticky přihlásí.

PIN se nastavuje v Admin → Uživatelé → tlačítko **PIN** u každého operátora (4–8 číslic).

### Admin
Na login stránce klikni **↓ Admin přihlášení** → zadej username + heslo.

---

## Instalace

```
1. Python 3.11+ z python.org — zatrhnout "Add Python to PATH"
2. Rozbalit ZIP do C:\printserver_win\
3. CMD jako správce ve složce projektu
4. install.bat
5. start.bat
6. http://localhost:5000
```

---

## Nastavení tiskáren

Admin → záložka **Nastavení** — každá tiskárna má vlastní kartu:

| Parametr | Popis |
|---|---|
| Název tiskárny | Přesný název z Windows — ze seznamu v UI |
| Protokol | TSPL (čínské tiskárny) nebo ZPL (Zebra) |
| DPI | 200 (LabelPrinter 4xx TSC), 203 nebo 300 (Zebra ZD420/ZD421) |
| Kódování | UTF-8, Latin-1, CP1250 |

Změny platí okamžitě bez restartu serveru.

**Diagnostika:**
```
venv\Scripts\python tools\detect_printer.py
```

**RAW port (bez ovladače):**
```
Nastav v UI: \\.\USB001
```

---

## Produkty a strany tiskáren

| Pole | Popis |
|---|---|
| Kód produktu | Identifikátor (skenuje se čtečkou) |
| QR obsah | Data zakódovaná do QR kódu — mezery na začátku/konci se zachovávají |
| Textový obsah (text1) | Hlavní textový popis produktu |
| Text 2 / Text 3 / Text 4 | Další volitelná textová pole |
| Strana | LEVÁ / PRAVÁ / OBĚ |
| Šablona | Přiřazená šablona štítku |

Naskenování produktu na špatnou stranu → červená chyba, nelze potvrdit.

Produkt bez šablony → oranžové varování v admin tabulce (⚠ není).

Šablonu nelze smazat pokud ji používá produkt — nejprve odeber šablonu z produktů.

**Duplikace produktu:** Admin → Produkty → **DUPLIKOVAT** vytvoří nový produkt se stejným QR obsahem, texty, stranou i šablonou. Kód nové kopie se vytvoří automaticky jako `PŮVODNÍ_KÓD COPY`; pokud už existuje, použije se `COPY 2`, `COPY 3` atd. Po vytvoření se kopie rovnou otevře pro úpravu.

### Import / Export CSV

**Export:** Admin → Produkty → **⬇ EXPORT CSV** — stáhne `produkty_export.csv` (UTF-8 s BOM, otevře se správně v Excelu).

**Import:** Admin → Produkty → **⬆ IMPORT CSV** — zobrazí náhled, duplikáty se přeskočí.

Formát CSV:
```
product_code,qr_content,text_content,text2,text3,text4,side,template_id
ABC001,https://erp/abc001,Popis produktu,druhý text,třetí text,,L,1
```

Povinné sloupce: `product_code`, `qr_content`. Ostatní jsou volitelné.

---

## Šablony štítků

Editor v Admin → Šablony. Pozice v **mm**.

### Prvky
- **QR kód** — X, Y, šířka, výška (mm)
- **Text** — X, Y, šířka, výška (mm)

### Velikost textu (TSPL)

Velikost určuje **výška okýnka (h mm)**.

| h v editoru | Výsledná výška textu (200 DPI) |
|---|---|
| < 4 mm | ~1 mm |
| 4–5 mm | ~1.5 mm |
| 6–7 mm | ~2 mm |
| ≥ 8 mm | ~3 mm |

### Proměnné

| Proměnná | Hodnota při tisku |
|---|---|
| `{product_code}` | Kód produktu |
| `{qr_content}` | Obsah QR kódu |
| `{text_content}` | Textový popis (= text1) |
| `{text1}` | Textový popis (alias pro text_content) |
| `{text2}` | Druhé textové pole |
| `{text3}` | Třetí textové pole |
| `{text4}` | Čtvrté textové pole |
| `{date}` | Datum tisku (dd.mm.yyyy) |
| `{time}` | Čas tisku (hh:mm:ss) |
| `{datetime_iso}` | Datum a čas (yyyy-mm-ddThh:mm:ss) |
| `{operator}` | Přihlášený uživatel v době tisku |

### TSPL generátor

- `SIZE` a `GAP` → mm
- `TEXT` a `QRCODE` souřadnice → dots (mm × DPI / 25.4)
- Font v uvozovkách: `TEXT 189,16,"4",0,1,1,"text"`

**Převodní tabulka 200 DPI:**

| mm | dots |
|---|---|
| 2 | 16 |
| 5 | 39 |
| 10 | 79 |
| 20 | 157 |
| 30 | 236 |

### ZPL generátor (Zebra)

- `^PW` a `^LL` → šířka a výška v dots
- `^FO` → pozice v dots
- `^BQN,2,X` → QR kód, X = velikost modulu (počítá se automaticky z šířky prvku)
- `^A0N,h,h` → text, h = výška v dots

---

## Výrobní příkazy

### Zahájení
1. Naskenovat produkt(y)
2. Volitelně zadat číslo výrobního příkazu (pole se vždy vymaže při otevření modalu)
3. ZAHÁJIT VÝROBU →

### Banner na hlavní obrazovce
- Číslo VP, operátor, čas zahájení
- Pokud číslo VP není zadané, zobrazí se **Bez zakázky**
- Počítadla kusů L / P / Celkem (obnovují se každých 10s)
- Kódy produktů

### Ukončení
- **UKONČIT VÝROBU** — řádné ukončení, uloží s počty kusů, bez potvrzovacího okna
- **ZRUŠIT VP** — pro zaseknuté příkazy, uloží jako ZRUŠEN, vyžaduje potvrzení

VP přežije restart serveru — obnoví se automaticky z DB včetně print_state.

### Historie
Admin → **Výrobní příkazy** — statusy AKTIVNÍ / UKONČEN / ZRUŠEN.

---

## UX flow — scan skenerem

```
Přihlášení (PIN nebo admin) → Modal výběru (nebo obnova aktivního VP)
    │
    ├── Scan levého → Enter
    │     ├── OK: focus skočí na pravé pole
    │     └── CHYBA: červené pole, po 1.5s vymaže, focus zpět
    │
    └── Scan pravého → Enter
          ├── OK: po 0.6s automaticky potvrdí
          └── CHYBA: červené pole, po 1.5s vymaže, focus zpět

Hlavní obrazovka
    ├── GPIO / HTTP trigger → automatický tisk
    └── Tlačítko TISKNOUT → ruční tisk
```

Při otevření modalu se vždy vymažou oba předchozí výběry i pole čísla VP.

Operátor v tisku = aktuálně přihlášený uživatel (ne ten kdo skenoval).

---

## Trigger na Windows

### HTTP (výchozí)
```
POST http://localhost:5000/api/debug/gpio/L
POST http://localhost:5000/api/debug/gpio/R
```
Nebo Debug panel → GPIO → SIMULOVAT.

### COM port (USB-Serial adaptér)
```python
# config.py
GPIO_MODE              = 'serial'
GPIO_SERIAL_PORT_LEFT  = 'COM3'
GPIO_SERIAL_PORT_RIGHT = 'COM4'
```

---

## Automatický start (Task Scheduler)

### Aktuální konfigurace
```
Název:      HessPrintServer
Program:    C:\printserver_win\venv\Scripts\python.exe
Argumenty:  app.py
Složka:     C:\printserver_win
Trigger:    Při spuštění počítače
```

### Doporučené nastavení pro provoz bez přihlášení
```
taskschd.msc → HessPrintServer → Vlastnosti
  → General → "Run whether user is logged on or not"
  → General → "Run with highest privileges"
  → Triggers → Edit → Delay task for: 30 seconds
```

### Kiosk režim — automatické přihlášení + spuštění prohlížeče

**1. Automatické přihlášení Windows účtu:**
```bat
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" /v AutoAdminLogon /t REG_SZ /d 1 /f
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" /v DefaultUserName /t REG_SZ /d "User" /f
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" /v DefaultPassword /t REG_SZ /d "heslo" /f
```

**2. Automatické otevření prohlížeče po přihlášení:**
```bat
schtasks /create /tn "HessBrowser" /tr "chrome.exe --start-maximized http://localhost:5000" /sc onlogon /ru "User" /delay 0:00:20
```

### Zastavení serveru
```bat
taskkill /f /im python.exe
:: nebo Task Scheduler → HessPrintServer → Ukončit
```

---

## Správa a monitoring

```bat
:: Logy
data\logs\printserver.log

:: Live log (PowerShell)
Get-Content data\logs\printserver.log -Wait -Tail 20

:: Ověření portu
netstat -ano | findstr :5000

:: Ověření sloupců DB
venv\Scripts\python -c "from models import get_db; db=get_db(); print([r[1] for r in db.execute('PRAGMA table_info(products)').fetchall()])"

:: Debug panel
http://localhost:5000/debug
```

**PRN soubory** — `data\last_print_L.prn` a `data\last_print_R.prn` obsahují poslední tiskové příkazy. Lze otevřít v poznámkovém bloku pro kontrolu.

---

## Časté problémy

| Problém | Řešení |
|---|---|
| `python` není rozpoznán | Přeinstaluj Python se zatrženým "Add to PATH" |
| Tiskárna se nezobrazuje v Nastavení | Zkontroluj instalaci v Windows → Nastavení → Tiskárny |
| Port 5000 obsazen | Změň port: poslední řádek `app.py` → `serve(app, port=8080)` |
| Tiskárna tiskne jen QR, bez textu | Debug → PRN náhled → zkontroluj TEXT/^FD příkazy |
| Pozice textu neodpovídají | Zkontroluj DPI v Admin → Nastavení |
| QR kód mění velikost při změně šablony | Zkontroluj že máš aktuální `printer.py` (oprava `^BQN` hardcode) |
| Šablonu nelze smazat | Odeber šablonu ze všech produktů v Admin → Produkty |
| `win32print` chyba | `venv\Scripts\python venv\Scripts\pywin32_postinstall.py -install` |
| Prázdná DB | Spustit `reset_db.bat` |
| Po restartu nejde tisknout (VP běží) | Opraveno v aktuální verzi — print_state se obnoví správně |
| Názvy tiskáren ukazují starý název | Admin → Nastavení → přeulož název tiskárny |
| PIN operátora nefunguje | Admin → Uživatelé → PIN — zkontroluj že je nastaven (✓ nastaven) |
| `text2 is not defined` | Zkontroluj že máš aktuální `admin.js` |
